...
def anivault_api_list():
   ...
   # TODO: Query anime from database where deleted_at is NULL 
   # TODO: Sort using SQLAlchemy's order_by() 
   # TODO: Serialize to dict and return JSON

   result = (Anime
             .query.filter(Anime.deleted_at.is_(None))
             .order_by(Anime.title_english, Anime.year)
             .all()
             )
   
   # anime_list = [i.to_dict() for i in result]
   anime_list = []
   for i in result:
       anime_list.append(i.to_dict())
